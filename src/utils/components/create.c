...
components_t *malloc_components(void)
{
    ...
    if (cmp == NULL)
        return my_puterr("Error : malloc component structure",
...
collider_t *malloc_collider(void)
{
    ...
    if (new == NULL)
        return my_puterr("Error : malloc collider structure",
        __FILE__, __LINE__).ptr;
...
animation_t *malloc_animations(void)
{
    ...
    if (anim == NULL)
        return my_puterr("Error : malloc animations structure",
        __FILE__, __LINE__).ptr;
    anim->toggle = OFF;
    anim->actual = NULL;
    anim->actual_frame = 0;
...
anim_t *malloc_anim_frame(void)
{
    ...
    if (anim == NULL)
        return my_puterr("Error : malloc anim frame node",
        __FILE__, __LINE__).ptr;
    anim->toggle = OFF;
    anim->frames_count = 0;
    anim->frame = NULL;
...
frame_t *malloc_frames_array(int count)
{
    ...
    if (frames == NULL )
        return my_puterr("Error : malloc frames array",
        __FILE__, __LINE__).ptr;
